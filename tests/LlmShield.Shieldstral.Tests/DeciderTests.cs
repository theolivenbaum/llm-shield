...
public class DeciderTests
{
    ...
    {
        string prefix = ShieldstralDecider.RenderPrefix(Refund, "Policy: refunds need a receipt. No receipt.");
        Assert.Equal(
        ...
        Assert.Equal(
            ...
            ShieldstralDecider.RenderSuffix(Refund, Refund.Options[1]));
    }

    [Fact]
    public async Task BranchesMatchRunningEachContinuationAlone()
    {
        if (Fixtures.ModelPath is not { } path) { _output.WriteLine("SHIELDSTRAL_MODEL is not set; skipping"); return; }
        var options = new ParallelOptions();
        using var model = new MinistralModel(path);

        int[] prefix = [.. model.Tokenizer.Encode(ShieldstralDecider.RenderPrefix(Refund,
        ...
        int[][] branches = [.. Refund.Options.Select(o =>
            model.Tokenizer.Encode(ShieldstralDecider.RenderSuffix(Refund, o), addSpecial: false).ToArray())];
    ...
    {
        if (Fixtures.ModelPath is not { } path) { _output.WriteLine("SHIELDSTRAL_MODEL is not set; skipping"); return; }
        using var decider = await ShieldstralDecider.OpenAsync(path);
