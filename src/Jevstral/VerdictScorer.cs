...
/// <summary>One question in the base checkpoint's native instruct/query/document format.</summary>
...
/// <param name="Document">The content the question is about.</param>
public readonly record struct VerdictRequest(string Instruct, string Query, string Document);

/// <summary>The base checkpoint's yes/no answer to one request.</summary>
/// <param name="Score">
/// Probability of "yes", in [0, 1]. It is the softmax of
...
public readonly record struct VerdictResult(
    float Score, float YesLogit, float NoLogit, int PromptTokens, int PrefilledTokens)
{
    ...
    public bool IsYes => Score > 0.5f;

    /// <summary>Verdict at a caller-chosen threshold.</summary>
    public bool IsYesAt(float threshold) => Score > threshold;

    public override string ToString()
        => $"{(IsYes ? "YES" : "no")} score={Score:F6} (yes={YesLogit:F4} no={NoLogit:F4})";
}

/// <summary>
/// The base checkpoint's native verdict: one instruct/query/document request, one yes/no.
///
/// Jevstral is built on Mistral's Shieldstral 1.0 3B. This is the question format that
/// checkpoint was trained on, kept as the primitive the fixtures and parity tests check
/// against. Typed decisions go through <see cref="JevstralDecider"/>.
///
...
/// </summary>
public sealed class VerdictScorer : IDisposable
{
    ...
    private VerdictScorer(MinistralModel model, bool ownsModel, ParallelOptions options)
    {
        _model = model;
    ...
    /// </param>
    public static ValueTask<VerdictScorer> OpenAsync(
        string ggufPath,
    ...
    public static async ValueTask<VerdictScorer> OpenAsync(
        ...
        ParallelOptions? options = null)
    {
        var scorer = new VerdictScorer(model, ownsModel, options ?? new ParallelOptions());

        try
        {
            if (cacheSystemPrompt)
            {
                ReadOnlyMemory<int> prefix = scorer.BuildSystemPrefixTokens();
                scorer._prefix = prefixCachePath is null
                    ? await SystemPromptCache.CaptureAsync(model, prefix, scorer._options).ConfigureAwait(false)
                    : await SystemPromptCache.LoadOrCaptureAsync(model, prefix, prefixCachePath, scorer._options).ConfigureAwait(false);
            }
        }
        catch
        {
    ...
    public static async Task<VerdictScorer> CreateAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        async Task<VerdictScorer> OpenCachedAsync()
    ...
    /// </para>
    /// </summary>
    public ValueTask<VerdictResult> ScoreAsync(
        string instruct, string query, string document, ParallelOptions? options = null)
        => ScoreAsync(new VerdictRequest(instruct, query, document), options);

    public async ValueTask<VerdictResult> ScoreAsync(VerdictRequest request, ParallelOptions? options = null)
    {
    ...
    /// </summary>
    public async ValueTask<float[]> VerdictLogitsAsync(VerdictRequest request, ParallelOptions? options = null)
    {
        int[] tokens = Tokenize(request);
        int prefilled = PrepareCache(tokens);
    ...
    public string RenderPrompt(VerdictRequest request) => ChatTemplate.Render(
    [
    ...
    /// </summary>
    public static string FormatUserMessage(VerdictRequest request)
        => $"<Instruct>: {request.Instruct}\n\n<Query>: {request.Query}\n\n<Document>: {request.Document}";

    public int[] Tokenize(VerdictRequest request)
    ...
    /// </summary>
    private static VerdictResult FromLogits(float yes, float no, int promptTokens, int prefilled)
    {
        float max = MathF.Max(yes, no);
        float eYes = MathF.Exp(yes - max);
        float eNo = MathF.Exp(no - max);
        return new VerdictResult(eYes / (eYes + eNo), yes, no, promptTokens, prefilled);
    }
    ...
    /// </summary>
    public async ValueTask<string> TopVerdictTokenAsync(VerdictRequest request, ParallelOptions? options = null)
    {
        int[] tokens = Tokenize(request);
        int prefilled = PrepareCache(tokens);
        ReadOnlyMemory<float> logits = await _model.ForwardAsync(tokens.AsMemory(prefilled), options ?? _options).ConfigureAwait(false);
